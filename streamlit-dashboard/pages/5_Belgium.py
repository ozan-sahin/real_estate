...
st.set_page_config(page_title="Belgium Real Estate Market", page_icon="🧇", layout="wide")

# Create a connection object.
conn = st.connection("gsheets_belgium", type=GSheetsConnection)
...
series_city = df.location.value_counts()
...
most_popular_cities = df.location.value_counts()[df.location.value_counts() > 10].index.tolist()
...
def get_lat_lon( address: str) -> tuple:
    ...
    getLoc = loc.geocode(address + " Belgium")
    if getLoc:
...
with column1:
    ...
    with column1_2:
        ...
        st.metric(label="City", value=df.iloc[index].location)
...
df2 = df[df.location.isin(most_popular_cities)] \
...
df_price = df[df.location.isin(most_popular_cities)].groupby(["location"])["price"] \
          .agg(["mean"]) \
          .sort_values(by="mean") \
          .round(2)

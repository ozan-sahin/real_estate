...
st.set_page_config(page_title="Turkey Real Estate Market", page_icon="🧿", layout="wide")

# Create a connection object.
conn = st.connection("gsheets_turkey", type=GSheetsConnection)
...
with column5:
    ...
    tile.subheader(f"📈 {round(df.price_EUR_per_m2.mean().round()):,.0f} €/m²")
with column6:
...
series_city = df.mahalle.value_counts()
common_cities = series_city[series_city > 5].index.tolist()
...
st.dataframe(
    ...
    column_config={
        ...
        "price_EUR_per_m2" : st.column_config.NumberColumn('💎PricePerArea',format="%0f €/m²"),
        "price" : st.column_config.NumberColumn('💶Price',format="%.0f TL"),
        "price_EUR" : st.column_config.NumberColumn('💶Price €',format="%.0f €"),
...
def get_lat_lon( address: str) -> tuple:
    ...
    getLoc = loc.geocode(address + " Turkey")
    if getLoc:
...
with column1:
    ...
    with column1_1:
        ...
        # if pd.notna(df.iloc[index].title):
        #     with st.expander("See description"):
        #         st.write(df.iloc[index].title)
    with column1_2:
        st.metric(label="Price", value=f"{df.iloc[index].price_EUR:,.0f} €")
        st.metric(label="Area", value=f"{df.iloc[index].area:,.0f} m²")
        st.metric(label="City", value=df.iloc[index].ilce)
        
    with column1_3:
        st.metric(label="Price per m²", value=f"{df.iloc[index].price_EUR_per_m2:,.0f} €/m²", delta=f"{df.iloc[index].sale_ratio * -1} %", delta_color="inverse")
        st.metric(label="Reference price per m²", value=f"{df.iloc[index].ref_price_per_m2 / 37:,.0f} €/m²")
        st.metric(label="Address", value=df.iloc[index].mahalle)
        st.metric(label="Type", value=df.iloc[index].real_estate_type)

...
st.set_page_config(page_title="UK Real Estate Market", page_icon="🏰", layout="wide")
...
ordered_columns = ['image', 'location', 'county', 'price_EUR', 'area', \
...
st.dataframe(
    ...
    column_config={
        ...
        "county" : st.column_config.TextColumn('🌍City'),
        "location" : st.column_config.TextColumn('🏘️Neighborhood'),
...
df_price = df[df.county.isin(most_popular_cities)].groupby(["county"])["price_EUR"] \
          .agg(["mean"]) \
          .sort_values(by="mean") \
          .round(2)

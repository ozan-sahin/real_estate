...
st.set_page_config(page_title="France Real Estate Market", page_icon="🥐", layout="wide")

# Create a connection object.
conn = st.connection("gsheets_france", type=GSheetsConnection)

st.title("🥐 France Real Estate Market")
...
ordered_columns = ['image', 'location', 'municipality', 'price', 'area', \
...
series_city = df.municipality.value_counts()
...
ordered_columns = ['image', 'title', 'municipality', 'price', 'area', \
                   'price_per_m2', 'bedrooms', 'query_date', 'location','url']
...
def get_lat_lon( address: str) -> tuple:
    ...
    getLoc = loc.geocode(address + " France")
    if getLoc:
...
with column2:
    try:
        lat, lon = get_lat_lon(df.iloc[index].location)
        if lat and lon:
...
df_price = df[df.municipality.isin(most_popular_cities)].groupby(["municipality"])["price"] \
          .agg(["mean"]) \
          .sort_values(by="mean") \
          .round(2)

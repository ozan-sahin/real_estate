...
with column3:
    provinces = st.multiselect("Provinces", df.province.sort_values().unique().tolist(),[])
    all_options = st.checkbox("Select all provinces", value=True)
...
df_query = df.query("price >= @low_price and price <= @high_price") \
            .query("surface >= @low_area and surface <= @high_area") \
